...
class Facility_night_Importer(DbStep):
    def run_step(self, settings: dict):
        h.log('importing facility_night:')
        h.log(f"using settings: {str(settings)}")
        ...
        # import facility_night
        h.logBeginTask('import facility_night')
        if db.handle_conflicting_output_tables(['facility_night'], schema):
            import_step.import_geopackage(db.connection_string_old, os.path.join(directory, settings['filename']), schema, 
                table='facility_night', target_srid=GlobalSettings.get_target_srid(), geometry_types=['POINT', 'POLYGON'])
        h.logEndTask()
...
class Facility_child_Importer(DbStep):
    def run_step(self, settings: dict):
        h.log('importing facility_child:')
        h.log(f"using settings: {str(settings)}")
        ...
        # import facility_child
        h.logBeginTask('import facility_child')
        if db.handle_conflicting_output_tables(['facility_child'], schema):
            import_step.import_geopackage(db.connection_string_old, os.path.join(directory, settings['filename']), schema, 
                table='facility_child', target_srid=GlobalSettings.get_target_srid(), geometry_types=['POINT', 'POLYGON'])
        h.logEndTask()
...
def create_optional_importer(db_settings: DbSettings, import_type: str):
    ...
    if import_type == 'facility_night':
        return Facility_night_Importer(db_settings)
    if import_type == 'facility_child':
        return Facility_child_Importer(db_settings)    
