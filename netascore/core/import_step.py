...
class OsmImporter(DbStep):
    ...
    def run_step(self, settings: dict):
        ...
        # create dataset "facility_night"
        h.logBeginTask('create dataset "facility_night"')
        if db.handle_conflicting_output_tables(['facility_night'], schema):
            db.execute('''
                CREATE TABLE facility_night AS (
                    SELECT ST_Transform(way, %(target_srid)s)::geometry(Point, %(target_srid)s) AS geom
                    FROM osm_point
                    WHERE amenity IN ('bar','biergarten','restaurant','fast_food','pub','cinema','theatre',
                                    'atm', 'kiosk','bank', 'music_venue', 'events_venue', 'vending_machine')
                    UNION ALL
                    SELECT ST_Transform(way, %(target_srid)s)::geometry(Polygon, %(target_srid)s) AS geom
                    FROM osm_polygon
                    WHERE amenity IN ('bar','biergarten','restaurant','fast_food','pub','cinema','theatre',
                ...
                CREATE INDEX facility_night_geom_idx ON facility_night USING gist (geom);
            ''', {'target_srid': GlobalSettings.get_target_srid()})
            db.commit()
        h.logEndTask()

        # create dataset "facility_child"
        h.logBeginTask('create dataset "facility_child"')
        if db.handle_conflicting_output_tables(['facility_child'], schema):
            db.execute('''
                CREATE TABLE facility_child AS (
                    SELECT ST_Transform(way, %(target_srid)s)::geometry(Point, %(target_srid)s) AS geom
                    FROM osm_point
                    WHERE amenity IN ('kindergarten','childcare','school','playground','toy_library',
                                    'bench','shelter','toilets','drinking_water',
                                    'pharmacy','supermarket','bakery','ice_cream', 'kiosk', 'cafe', 'library', 'park', 'water_point', 'watering_place')
                    UNION ALL
                    SELECT ST_Transform(way, %(target_srid)s)::geometry(Polygon, %(target_srid)s) AS geom
                    FROM osm_polygon
                    WHERE amenity IN ('kindergarten','childcare','school','playground','toy_library',
                ...
                CREATE INDEX facility_child_geom_idx ON facility_child USING gist (geom);
            ''', {'target_srid': GlobalSettings.get_target_srid()})
            db.commit()
        h.logEndTask()
